...
lightened_colors = [start_color_blue, middle_color_green, end_color_red]

for i, c in enumerate(lightened_colors):
    pos = i * (-1)
    fc = np.dstack([c[0] * blue, c[1] * green, c[2] * red]) / 255.0
    fc_with_border = np.ones((fc.shape[0]+2, fc.shape[1]+2, fc.shape[2]))
    fc_with_border[1:-1, 1:-1] = fc
    fc = fc_with_border
    ax.plot_surface(x+i*3, y+i, np.full_like(x, pos), rstride=1, cstride=1, facecolors=fc,  linewidth=1, shade=False)


ax.view_init(elev=110, azim=-90)
ax.set_axis_off()

plt.savefig('optical.png', transparent=True)
#plt.show()
